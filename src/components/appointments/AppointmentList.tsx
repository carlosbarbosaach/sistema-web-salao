    ...
    <aside className="rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="p-4">
        <div className="rounded-2xl border border-slate-200 overflow-hidden">
          {/* subheader (fixo ao rolar) */}
          <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-3 bg-slate-50">
            <span className="text-sm font-medium text-slate-700">Agendamentos do dia</span>
          ...
          {count > 0 ? (
            <ul className="divide-y divide-slate-200 max-h-96 overflow-auto" aria-live="polite">
              {items.map((a) => {
                const digits = a.phone.replace(/\D/g, "");
                return (
                  <li key={a.id} className="px-4 py-3 hover:bg-slate-50 transition-colors">
                    <div className="flex items-center gap-3.5">
                      {/* avatar com acento indigo suave */}
                      <div
                        className="flex h-9 w-9 items-center justify-center rounded-full bg-indigo-50 text-indigo-700 text-xs font-semibold"
                        aria-hidden="true"
                      >
                        {initials(a.client)}
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <a
                          ...
                          className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300"
                          title="WhatsApp"
                        >
                          <WhatsIcon />
                        </a>
